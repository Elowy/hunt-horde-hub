...
DROP TRIGGER IF EXISTS log_animal_changes ON public.animals;

-- Update the log_animal_activity function to capture detailed changes
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.condition IS DISTINCT FROM NEW.condition THEN
      changes := changes || jsonb_build_object('condition', jsonb_build_object('old', OLD.condition, 'new', NEW.condition));
    END IF;
    ...
    IF OLD.vet_check IS DISTINCT FROM NEW.vet_check THEN
      changes := changes || jsonb_build_object('vet_check', jsonb_build_object('old', OLD.vet_check, 'new', NEW.vet_check));
    END IF;
...
-- Create trigger for animal changes
CREATE TRIGGER log_animal_changes
  AFTER INSERT OR UPDATE OR DELETE ON public.animals
  FOR EACH ROW
  EXECUTE FUNCTION public.log_animal_activity();