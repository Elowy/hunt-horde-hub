...
export const GuestRegistrationQRDialog = () => {
  ...
  const generateQRCode = async () => {
    ...
    try {
      ...
      // Deactivate existing guest registration QR codes
      if (qrData) {
        await supabase
          .from("qr_codes")
      ...
      const newCode = `GR-${user.id.slice(0, 8)}-${Date.now()}`;
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          name: "Vendég beiratkozási QR kód",
          is_active: true,
          user_id: user.id,
        })
        .select()
  ...
  const downloadQRCode = () => {
    ...
    const svg = document.querySelector("#qr-guest-registration");
    if (!svg) return;
    ...
    img.onload = () => {
      ...
      downloadLink.download = "vendeg-beiratkozas-qr.png";
  ...
  const copyURL = () => {
    ...
    const url = `${window.location.origin}/guest-registration?code=${qrData.code}`;
  ...
  const qrUrl = qrData ? `${window.location.origin}/guest-registration?code=${qrData.code}` : "";
          ...
          {!qrData ? (
            <div className="text-center py-8">
              <p className="text-muted-foreground mb-4">
                Még nincs aktív vendég beiratkozási QR kód.
              </p>
